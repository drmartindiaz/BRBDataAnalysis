---
title: "Healthcare Worker Analysis - WOMBAT Study (v2)"
...
DOCTOR_NURSE_FORM_BRB_v2 <- read_csv("data/DOCTOR-NURSE_FORM_BRB_v2_from_23_03_2025_to_30_04_2025.csv", 
                                     col_types = cols(`session date` = col_character(), 
                                                      `session start` = col_time(format = "%H:%M:%S"), 
                                                      `session end` = col_time(format = "%H:%M:%S"), 
                                                      `start time` = col_time(format = "%H:%M:%S"), 
...
wombat_type_professional <- read_delim("data/wombat_type_professional.csv", 
...
data_wmbt_time <- DOCTOR_NURSE_FORM_BRB_v1 %>%
  ...
  filter(!`session id` == 10195,
         !`observer/session/task id` == "10175_10260_60") %>% # seems like a test session 
  left_join(wombat_type_professional, join_by(`participant id` == healthcare_work_id_v2)) 
...
tipos_profesional <- data_wmbt_time %>% 
  ...
  distinct(`participant id`, Metric) %>% count(Metric, name = "Count") %>% 
  mutate(Metric = case_when(Metric == "Nurse" ~ "- Nurses",
                            Metric == "Physician" ~ "- Physicians",
                            Metric == "Other" ~ "- Others",
                              TRUE ~ Metric)) %>% 
  arrange(desc(Count))
...
tool_time_summary <- data_wmbt_time_2 %>%
  ...
  summarise(
    ...
    computer_time = sum(total_activity * (`[How] Desktop Computer`), na.rm = TRUE),
    mobile_time = sum(total_activity * (`[How] Mobile Computer`), na.rm = TRUE),
    phone_time = sum(total_activity * `[How] Phone`, na.rm = TRUE),
    total_time = sum(paper_time, computer_time, mobile_time,phone_time, na.rm = TRUE),
    .groups = 'drop'
  ) %>%
  mutate(
    across(ends_with("_time"), ~ round(., 2)),
    pct_paper = round((paper_time / total_time) * 100, 1),
    pct_computer = round((computer_time / total_time) * 100, 1),
    pct_mobile = round((mobile_time / total_time) * 100, 1),
...
#   group_by(What, with_whom) %>% 
#   summarise(
#     activity_time = sum(total_activity, na.rm = TRUE),
#     .groups = 'drop'
#   ) 
#   
# ggplot(tool_time_summary_nurse, aes(x = activity_time, y = with_whom)) +
#   geom_col(fill = "steelblue") +
#   labs(title = "Frecuencia de Interacciones por Rol")
...
loc_table <- loc_time_summary %>%
  select(What, total_time,n_act,
    pct_corridors, pct_workstation, pct_observation_room, pct_triage_Area, pct_treatment_Room, pct_other, pct_ambulance_Bay, pct_waiting_Area, pct_xray_CT, pct_registration, pct_plaster_Room, pct_medication_Room, pct_consultation_Room) %>%
...
### Analysis by Healthcare Professional Type
...
n2 <- ggplot(filter(sc_activity_time_nurse, What == "Documentation"), 
             aes(x = "", y = pct_time, fill = Subcategories)) +
  geom_col(width = 1, color = "white") +
  coord_polar("y") +
  geom_text(aes(label = paste0(pct_time, "%")), 
...
n3 <- ggplot(filter(sc_activity_time_nurse, What == "Indirect Care"), 
             aes(x = "", y = pct_time, fill = Subcategories)) +
  geom_col(width = 1, color = "white") +
  coord_polar("y") +
...
n4 <- ggplot(filter(sc_activity_time_nurse, What == "Medication Tasks"),
             aes(x = "", y = pct_time, fill = Subcategories)) +
  geom_col(width = 1, color = "white") +
  coord_polar("y") +
  geom_text(aes(label = paste0(pct_time, "%")), 
...
n5 <- ggplot(filter(sc_activity_time_nurse, What == "No observation"), 
             aes(x = "", y = pct_time, fill = Subcategories)) +
  geom_col(width = 1, color = "white") +
  coord_polar("y") +
  geom_text(aes(label = paste0(pct_time, "%")), 
...
n6 <- ggplot(filter(sc_activity_time_nurse, What == "Other"), 
             aes(x = "", y = pct_time, fill = Subcategories)) +
  geom_col(width = 1, color = "white") +
  coord_polar("y") +
  geom_text(aes(label = paste0(pct_time, "%")), 
